...
for l in ivalues(Letters) do
	...
	if l == "BACK" or l == "ENTER" then
		t[#t].File = THEME:GetPathF("", "_venacti 26px normal");
	end;
end;
t[#t+1] = LoadActor( THEME:GetPathS(Var "LoadingScreen", "type key") ) .. { Name = "Type"; SupportPan = true; }
t[#t+1] = LoadActor( THEME:GetPathS(Var "LoadingScreen", "back") ) .. { Name = "Back"; SupportPan = true; }
t[#t+1] = LoadActor( THEME:GetPathS(Var "LoadingScreen", "enter") ) .. { Name = "Enter"; SupportPan = true; }
t[#t+1] = LoadActor( THEME:GetPathS(Var "LoadingScreen", "move cursor") ) .. { Name = "Move"; SupportPan = true; }
...
for pn in ivalues(PlayerNumber) do
	t[#t+1] = LoadActor( THEME:GetPathG("_frame", "1D"),
		...
		OnCommand = cmd(
				zoom,0;rotationz,-360*2;
				sleep,0.55;
				decelerate,0.5;zoom,1;rotationz,0;
			);
		...
		TweenOffCommand = cmd(accelerate,0.25;zoomy,0;addy,-15 * (pn == PLAYER_1 and 1 or -1));
	};
end
...
t.BeginCommand = function(self)
	...
	(cmd(x,-80;diffuse,color("#FFFF00")))(Keys["BACK"].Text);
	(cmd(x,80;diffuse,color("#FFFF00")))(Keys["ENTER"].Text);
...
t.CodeMessageCommand = function(self, param)
...
t.SelectKeyMessageCommand = function(self, param)
	local key = param.Key;
	local pn = param.PlayerNumber;
...
t.MenuTimerExpiredMessageCommand = function(self, param)
	for pn in ivalues(PlayerNumber) do
...
t.OnCommand = function(self, param)
...
t.OffCommand = function(self, param)
	for key in ivalues(Letters) do
		local fDist = distance( Keys[key].Text:GetX(), Keys[key].Text:GetY(), 0, 100 );
